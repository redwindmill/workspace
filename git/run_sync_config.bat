...
SET PS_COMMAND="(Get-Content '%PATH_SCRIPT_DIR%\nix.gitconfig')^
	-replace '\$\{GIT_FULLNAME\}', '%GIT_FULLNAME%'^
	-replace '\$\{GIT_EMAIL\}', '%GIT_EMAIL%'^
	-replace 'autocrlf = input', 'autocrlf = true'^
	| Set-Content -Path '%userprofile%\.gitconfig' -Encoding Ascii"
